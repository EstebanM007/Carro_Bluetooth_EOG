...
class App:
    ...
    def __init__(self, master):
        ...
        """
        self.master = master
        master.title("Interfaz de Configuración LSL y COM - Versión Mejorada con Pruebas")
        
        # Inicializar variables importantes al principio
        self.running = False
        self.log_viewers = {}
        ...
        self.test_serial_connection = None
        self.available_serial_ports = []
        self.available_lsl_streams = []
        self.condition_rows = []  # Mover esta inicialización aquí
        self.quick_command_buttons = []  # Añadir esta inicialización
        ...
        # NUEVA SECCIÓN: Frame de prueba de comandos seriales
        self.frame_test = tk.LabelFrame(self.frame_serial, text="Prueba de Comandos Serial")
        self.frame_test.grid(row=4, column=0, columnspan=3, padx=10, pady=5, sticky="we")

        # Fila de pruebas
        test_row = tk.Frame(self.frame_test)
        test_row.pack(fill="x", padx=5, pady=5)

        tk.Label(test_row, text="Comando:").pack(side="left", padx=5)
        self.test_command_entry = tk.Entry(test_row, width=10)
        self.test_command_entry.pack(side="left", padx=5)
        self.test_command_entry.bind("<Return>", self.send_test_command)

        self.btn_send_test = tk.Button(test_row, text="Enviar", command=self.send_test_command)
        self.btn_send_test.pack(side="left", padx=5)

        self.btn_test_connect = tk.Button(test_row, text="Conectar Puerto", command=self.connect_test_serial)
        self.btn_test_connect.pack(side="left", padx=5)

        self.btn_test_disconnect = tk.Button(test_row, text="Desconectar", command=self.disconnect_test_serial, state="disabled")
        self.btn_test_disconnect.pack(side="left", padx=5)

        self.lbl_test_status = tk.Label(test_row, text="Puerto: Desconectado", fg="red")
        self.lbl_test_status.pack(side="left", padx=10)

        # Botones de comandos rápidos
        self.quick_commands_row = tk.Frame(self.frame_test)
        self.quick_commands_row.pack(fill="x", padx=5, pady=2)

        tk.Label(self.quick_commands_row, text="Comandos rápidos:").pack(side="left", padx=5)
        
        # Inicializar lista de botones rápidos
        self.quick_command_buttons = []
        self.update_quick_command_buttons()

        # Consola serial
        self.serial_console_frame = tk.LabelFrame(self.frame_serial, text="Consola Serial")
        self.serial_console_frame.grid(row=5, column=0, columnspan=3, padx=10, pady=0, sticky="we")
        ...
        self.btn_clear_console.grid(row=6, column=0, columnspan=3, padx=5, pady=5, sticky="we")
        ...
        # Botón para actualizar comandos rápidos
        self.btn_update_commands = tk.Button(btn_frame, text="Actualizar Comandos Rápidos", 
                                           command=self.update_quick_command_buttons)
        self.btn_update_commands.pack(side="left", padx=5)
        ...
        self.condition_rows = []
        self.refresh_all()
        self.update_quick_command_buttons()  # Ahora es seguro llamar a este método
        self.master.protocol("WM_DELETE_WINDOW", self.on_closing)

    def update_quick_command_buttons(self):
        """Actualiza los botones de comandos rápidos basados en la configuración actual."""
        # Limpiar botones existentes
        for btn in self.quick_command_buttons:
            btn.destroy()
        self.quick_command_buttons = []

        # Obtener comandos únicos de la configuración actual
        commands = set()
        for row in self.condition_rows:
            pos_letter = row.pos_letter.get().strip()
            neg_letter = row.neg_letter.get().strip()
            if pos_letter:
                commands.add(pos_letter)
            if neg_letter:
                commands.add(neg_letter)

        # Crear botones para cada comando
        for cmd in sorted(commands):
            if cmd:  # Asegurarse de que no esté vacío
                btn = tk.Button(self.quick_commands_row, text=cmd, width=3, 
                              command=lambda c=cmd: self.send_quick_command(c))
                btn.pack(side="left", padx=2)
                self.quick_command_buttons.append(btn)

    def send_quick_command(self, command):
        """Envía un comando rápido."""
        self.test_command_entry.delete(0, tk.END)
        self.test_command_entry.insert(0, command)
        self.send_test_command()

    def connect_test_serial(self):
        """Conecta al puerto serial solo para pruebas."""
        if not self.simulate_serial.get():
            ...
            try:
                ...
                self.btn_test_disconnect.config(state="normal")
                self.append_serial_console(f"Conectado a {port} @ {baudrate} bps para pruebas\n")
            ...
            self.lbl_test_status.config(text="Puerto: Simulado", fg="blue")
            self.btn_test_connect.config(state="disabled")
            self.btn_test_disconnect.config(state="normal")
            self.append_serial_console("Conectado en modo simulación para pruebas\n")

    def disconnect_test_serial(self):
        """Desconecta el puerto serial de pruebas."""
        if self.test_serial_connection and not self.simulate_serial.get():
            try:
        ...
        self.test_serial_connection = None
        self.lbl_test_status.config(text="Puerto: Desconectado", fg="red")
        self.btn_test_connect.config(state="normal")
        self.btn_test_disconnect.config(state="disabled")
        self.append_serial_console("Desconectado del puerto de pruebas\n")

    def send_test_command(self, event=None):
        """Envía un comando de prueba al puerto serial."""
        if not self.test_serial_connection:
            messagebox.showwarning("Advertencia", "Primero conecte al puerto serial para pruebas.")
            return
        
        command = self.test_command_entry.get().strip()
        if not command:
            messagebox.showwarning("Advertencia", "Ingrese un comando para enviar.")
            return
        
        try:
            # Enviar comando
            self.test_serial_connection.write(command.encode())
            
            # Mostrar en consola
            self.append_serial_console(f"PRUEBA - Enviado: '{command}'\n")
            
            # Si no es simulación, intentar leer respuesta
            if not self.simulate_serial.get() and hasattr(self.test_serial_connection, 'in_waiting'):
                # Esperar brevemente por respuesta
                time.sleep(0.1)
                if self.test_serial_connection.in_waiting > 0:
                    response = self.test_serial_connection.read(self.test_serial_connection.in_waiting)
                    try:
                        response_str = response.decode('utf-8', errors='ignore')
                        if response_str.strip():
                            self.append_serial_console(f"RESPUESTA: {response_str}")
                    except Exception:
                        self.append_serial_console(f"RESPUESTA (bytes): {response}\n")
            ...
            self.append_serial_console(f"ERROR enviando '{command}': {e}\n")
...
if __name__ == "__main__":
    root = tk.Tk()
    root.geometry("1200x900")  # Ventana más grande para acomodar nuevas funciones
    app = App(root)
    ...
    tools_menu.add_command(label="Actualizar Comandos Rápidos", command=app.update_quick_command_buttons)
    
    # Menú Pruebas
    test_menu = tk.Menu(menubar, tearoff=0)
    menubar.add_cascade(label="Pruebas", menu=test_menu)
    test_menu.add_command(label="Conectar Puerto de Prueba", command=app.connect_test_serial)
    test_menu.add_command(label="Desconectar Puerto de Prueba", command=app.disconnect_test_serial)
    test_menu.add_separator()
    test_menu.add_command(label="Limpiar Consola", command=app.clear_console)
    ...
    help_menu.add_command(label="Acerca de", 
                         ...
                         "Interfaz LSL-Serial Mejorada v2.1\n\n"
                         "Funciones principales:\n"
                         "• Configuración de baudios personalizables\n"
                         "• Streams manuales por nombre\n"
                         "• Reconexión automática\n"
                         "• Estadísticas en tiempo real\n"
                         "• Prueba de comandos serial independiente\n"
                         "• Comandos rápidos configurables\n"
                         "• Mejor manejo de errores\n\n"
                         "NUEVA FUNCIONALIDAD:\n"
                         "• Sección de Prueba de Comandos Serial\n"
                         "• Conexión independiente para pruebas\n"
                         "• Botones de comandos rápidos\n"
